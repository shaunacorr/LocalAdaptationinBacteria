...
## Compare isolate origin within each medium ##

mixed_model <- lmer(auc_e ~ origin * media + (1|seaweed_rep) + (1|isolate), data = mixed_II, REML = FALSE)
summary(mixed_model)
anova(mixed_model)
emmeans(mixed_model, pairwise ~ origin | media)
confint(emmeans(mixed_model, pairwise ~ origin | media)$contrasts)
...
# Calculate proportion of isolates with higher growth in their native media
dulse_native_better <- sum(dulse$mediafucus < 0) / nrow(dulse)
fucus_native_better <- sum(fucus$mediafucus > 0) / nrow(fucus)
...
p2 <- ggplot() +
  ...
  labs(y = "Fitness (Average AUC)", x = "Host") +
  ylim(0, 1.4) +
