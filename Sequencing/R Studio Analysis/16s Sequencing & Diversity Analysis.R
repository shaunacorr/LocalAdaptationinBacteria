...
png("Figure2.png", width = 3500, height = 3500, res = 300)
...
shannon_long <- combined_df %>%
  pivot_longer(cols = c(Genus, Phylum),
               names_to = "Metric",
               values_to = "Value")
...
shannon_long$rep <- factor(shannon_long$rep, 
                           levels = c("Fucus: Uncultured", "Fucus: SDM", "Fucus: MB", "Palmaria: Uncultured", "Palmaria: SDM", "Palmaria: MB"))
library(dplyr)
library(stringr)
shannon_long <- shannon_long %>%
  mutate(Treatment = str_trim(str_extract(rep, "(?<=:).*")))

# Define the list of comparisons
comparisons <- list(
  c("Fucus: Uncultured", "Palmaria: Uncultured"),
  c("Fucus: Uncultured", "Fucus: SDM"),
  c("Fucus: Uncultured", "Fucus: MB"),
  c("Fucus: MB", "Fucus: SDM"),
  c("Palmaria: Uncultured", "Palmaria: SDM"),
  c("Palmaria: MB", "Palmaria: Uncultured"),
  c("Palmaria: MB", "Palmaria: SDM")
)

Figure1<-ggplot(shannon_long, aes(x = rep, y = Value, fill = Treatment)) +
  geom_boxplot() +
  facet_wrap(~Metric, scales = "free_y", nrow = 1) +
  theme_bw() +
  scale_fill_manual(values = c("Uncultured" = "#63C7B2", 
                               "SDM" = "#8E6C88",
                               "MB" = "#263D42")) +
  ...
  theme(text = element_text(size = 14), 
    ...
    axis.text.x = element_text(vjust = 0.5, hjust = 0.5,face = "bold.italic", size =14),
...
print(Figure1)  # Print the plot to the file
dev.off() 
...
NMDS_Plot <- phyloseq::plot_ordination(pseq_rarefy, ord.mds.wunifrac,
  ...
  scale_color_manual(name = "Treatment", values = c("MB" = "#263D42", "Uncultured" = "#63C7B2", "SDM" = "#8E6C88")) +  # Custom colors
  guides(size = FALSE) +
  theme_q2r() +
  theme(
    ...
    legend.text = element_text(size = 14),
    legend.title = element_text(size = 16)  # Set larger size for legend titles
  ) +
...
for (i in 1:nrow(df)) {
  ...
  group_vector1[i] <- ifelse(grepl("Dulse", df$Var1[i]) & grepl("Agar", df$Var1[i]), "Palmaria SDM",
                             ifelse(grepl("Dulse", df$Var1[i]) & grepl("MB", df$Var1[i]), "Palmaria MB",
                                    ifelse(grepl("Dulse", df$Var1[i]) & grepl("Microbiome", df$Var1[i]), "Palmaria Uncultured",
                                           ifelse(grepl("Fucus", df$Var1[i]) & grepl("Agar", df$Var1[i]), "Fucus SDM",
  ...
  group_vector2[i] <- ifelse(grepl("Dulse", df$Var2[i]) & grepl("Agar", df$Var2[i]), "Palmaria SDM",
                             ifelse(grepl("Dulse", df$Var2[i]) & grepl("MB", df$Var2[i]), "Palmaria MB",
                                    ifelse(grepl("Dulse", df$Var2[i]) & grepl("Microbiome", df$Var2[i]), "Palmaria Uncultured",
                                           ifelse(grepl("Fucus", df$Var2[i]) & grepl("Agar", df$Var2[i]), "Fucus SDM",
...
subset_df_dulse <- subset(df, group1 == "Palmaria Uncultured" &
                      (group2 %in% c("Palmaria SDM", "Palmaria MB", "Palmaria Uncultured", "Fucus Uncultured")))
...
subset_df_dulse$group2 <- factor(subset_df_dulse$group2, levels = c("Palmaria Uncultured", "Palmaria SDM", "Palmaria MB", "Fucus Uncultured"))
...
subset_df_fucus <- subset(df, group1 == "Fucus Uncultured" &
                            (group2 %in% c("Fucus SDM", "Fucus MB", "Palmaria Uncultured", "Fucus Uncultured")))
...
subset_df_fucus$group2 <- factor(subset_df_fucus$group2, levels = c("Fucus Uncultured", "Fucus SDM", "Fucus MB", "Palmaria Uncultured"))
...
png("barplots.png", width = 3000, height = 1500, res = 300)
grid.draw(barplots)  # Draw the grob to the file
dev.off()
...
Figure1_labelled <- arrangeGrob(Figure1, top = label_a)
NMDS_labelled <- arrangeGrob(NMDS_Plot, top = label_b)

# Arrange plots without immediately plotting
Fig3 <- arrangeGrob(Figure1_labelled, NMDS_labelled)
Fig3
